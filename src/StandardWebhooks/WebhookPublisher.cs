...
/// </summary>
public sealed class WebhookPublisher(HttpClient httpClient, TimeProvider timeProvider, byte[] key, string? keyId = null)
{
    ...
    private string BuildSignatureHeader(string messageId, DateTimeOffset timestamp, ReadOnlySpan<byte> payload)
    {
        ...
        var tag = ComputeTag(_key, messageId, timestamp, payload);
        try
        {
            var sig = Base64UrlEncode(tag);
            var header = $"t={timestamp.ToUnixTimeSeconds().ToString(CultureInfo.InvariantCulture)}, v1={sig}";
    ...
    private static byte[] ComputeTag(ReadOnlySpan<byte> key, string msgId, DateTimeOffset timestamp,
        ReadOnlySpan<byte> payload)
    {
        ...
        try
        {
            using var ih = IncrementalHash.CreateHMAC(HashAlgorithmName.SHA256, key.ToArray());
